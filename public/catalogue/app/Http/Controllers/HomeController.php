...
class HomeController extends Controller
{
    ...
    public function display($id)
    {
        ...
        $likes =  DB::select('select COUNT(viewings.id) as likes from episodes join viewings on viewings.episode_id=episodes_id where viewings.like=1 and episodes.id=' . $id);
        $dislikes =  DB::select('select COUNT(viewings.id) as likes from episodes join viewings on viewings.episode_id=episodes_id where viewings.like=-1 and episodes.id=' . $id); 
        $dislike = $dislikes[0];
        $like = $likes[0];
